...
function Services({ data, site_info, og_info }) {
  return (
    <>
      <Head>
        <title>{`Services - ${
        ...
        <meta
          property="og:title"
          content={`Services - ${site_info?.site_info?.title}`}
        />
        <meta
...
export const getServerSideProps = async () => {
  ...
  const response = await fetch(
    `${process.env.NEXT_PUBLIC_API_PROTOCOL}://${process.env.NEXT_PUBLIC_API_HOST}/wp-json/stafford/v1/services/`
  );
